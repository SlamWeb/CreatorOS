...
- **可确认运营计划**：严格 `OperationPlan` 统一批量加选题和整列调序；Preview 只读，宿主确认后才在一个事务中执行，并用状态指纹拒绝过期确认。
...
1. **自然语言运营入口**：让 Provider 以 Structured Output 把用户表达解析成现有 `OperationPlan`，通过 Agent Tool/UI 展示 Preview，并由用户确认后提交。
