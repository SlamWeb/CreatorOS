...
export function RunsPage() {
  ...
  return <><div className="page-heading"><div><h1>运行记录</h1></div><span className="toolbar-note">共 {query.data.page.total} 次运行</span></div>{runs.length ? <div className="runs-list">{runs.map((run) => <Link to={`/runs/${run.id}`} className="run-card" key={run.id}><div className="run-card-main"><p className="run-context">{run.creator_name} · {run.series_name}</p><h3>{run.topic_title}</h3><p className="run-card-meta">版本 {run.active_revision_number} · 更新于 {formatDate(run.updated_at)}</p></div><StatusPill status={run.status} /><span className="row-arrow">→</span></Link>)}</div> : <EmptyState title="还没有运行记录">当栏目里有了选题并开始生产，运行记录会在这里出现。<br /><Link className="text-link" to="/creators">先查看账号目录 →</Link></EmptyState>}</>;
}
