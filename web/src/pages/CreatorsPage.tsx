...
export function CreatorsPage() {
  ...
  return <>
    <div className="page-heading"><div><h1>账号目录</h1></div><button className="button button-primary" onClick={() => setFormOpen((value) => !value)}>{formOpen ? "收起" : "创建账号"} <span>{formOpen ? "−" : "+"}</span></button></div>
    {formOpen ? <form className="inline-form" onSubmit={(event) => { event.preventDefault(); createMutation.mutate(); }}><div className="form-heading"><div><h2>建立一个运营边界</h2></div><span className="form-note">平台固定为小红书</span></div><div className="form-grid"><label>账号名称<input value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="例如：面试知识实验室" required maxLength={120} /></label><label>账号标识 <small>可选</small><input value={handle} onChange={(event) => setHandle(event.target.value)} placeholder="例如：agent_lab" maxLength={160} /></label></div>{createMutation.isError ? <p className="form-error">{createMutation.error.message}</p> : null}<div className="form-actions"><button className="button button-primary" type="submit" disabled={createMutation.isPending}>{createMutation.isPending ? "保存中…" : "保存账号"}</button><button className="button button-quiet" type="button" onClick={() => setFormOpen(false)}>取消</button></div></form> : null}
    {items.length ? <><div className="directory-toolbar"><span>已配置 {page.total} 个账号</span><span className="toolbar-note">数据来自本地 Studio API</span></div><div className="creator-grid creator-grid-wide">{items.map((creator) => <CreatorCard key={creator.id} creator={creator} />)}</div></> : <EmptyState title="还没有账号"><span>点击右上角“创建账号”，开始设置你的第一个内容栏目。</span><br /><Link className="text-link" to="/">回到今日 →</Link></EmptyState>}
  </>;
}
