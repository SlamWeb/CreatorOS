...
export function TodayPage() {
  ...
  return <>
    <div className="page-heading"><div><h1>今天要做什么</h1></div><div className="heading-actions"><Link className="button button-primary" to="/creators">查看账号目录 <span>→</span></Link></div></div>
    ...
    {!hasData ? <div className="first-use-grid"><EmptyState title="还没有运营账号"><span>先创建一个账号和栏目，再把选题送进生产。</span><br /><Link className="text-link" to="/creators">创建第一个账号 →</Link></EmptyState><div className="next-card"><h3>准备好你的第一个栏目</h3><p>确定定位与受众，加入想做的选题。生产完成后，在这里检查图片并决定是否批准。</p><Link className="text-link" to="/creators">查看账号目录 →</Link></div></div> : <>
