...
- 通过页面添加指定标题，列表显示“已入队 / 待开始”；这是用户要求的正式选题，不是隔离测试夹具。
- 点击“生产”后页面明确报错：“双 Skill 栏目生产尚未接入，当前只支持单生产 Skill 栏目。”与 `ContentRunService.create_run` 的 pair 守卫一致，未启动 Codex 或生图。
- 发现 P3 入口与生产能力不一致：pair 栏目仍显示可点击的生产按钮。P4 尚未完成；当前不能宣称网站已跑通双 Skill 生产，也不能据此比较单/双 Skill 的内容质量。
- 下一步需接入 P4 的一次委托、双 Skill 输入快照与中间产物验收，再复用本次已入队选题测试，避免重复添加。本轮没有修改生产实现或绕过守卫。
