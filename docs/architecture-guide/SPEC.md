...
## 2026-09-18 当前架构图与理解债学习切片

- 原 HTML 固定于 bb219dd，保留作历史；不把其中“Web 未复用 Loop”等旧限制当作当前事实。
- 本轮按42f640c代码核对，新增 current-map.md、架构图与生成提示词。只做学习材料，不改业务代码和正式数据，不触发项目生产/调研；架构插图按用户要求使用内置生图工具。
- 明确分开 Web/CLI 运营 Loop、候选选择/通用操作计划、ContentRun 执行器、Codex 内部 Agent。人工确认入队与产物人工验收是两个不同关口。
- 不把配置/Skill当作任意自动化能力；当前Web工具白名单不能直接完成所有账号/栏目CRUD。普通选题编辑经独立OperationPlan入口，研究候选选择由专用工具创建Preview。
- 学习节奏：整体任务复述 → 跟一次真实工具调用 → 上下文投影 → 生产恢复 → 终态评测。每轮一个问题、一段短代码或一条可观测轨迹，先预测再验证。
- 验收：图中中文与箭头目视检查，文字导览到代码的相对链接存在，新增材料不包含秘密；只提交本轮文档与资产。
- 完成：current-map.md含三角色任务走读、七模块代码地图、五次短课计划及前两题纠偏；architecture-20260918.png使用内置image_gen生成并复制入仓库，提示词另存。关键中文、双向工具结果/产物箭头及两个人工关口已目视核对，相对链接存在性检查通过。仅文档，无运行时代码变更，未启动正式调研/生产或发布。
